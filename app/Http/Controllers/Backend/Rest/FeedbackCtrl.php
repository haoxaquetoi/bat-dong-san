...
class FeedbackCtrl extends Controller {
    ...
    private function _validateId($id) {
        Validator::make(
                ['id' => $id], ['id' => 'required|exists:feedback,id'], [
    ...
    /**
     * Lay danh sach feedback từ client
     */
    function postFeedkback(FeedbackArticleModel $postFdModel, Request $request) {
        $request->readed = $request->readed ? (int) $request->readed : null;
        $request->freeText = $request->freeText ? $request->freeText : null;
        $request->fbID = $request->fbID ? $request->fbID : 0;
        $request->readed = ($request->readed == -1) ? NULL : $request->readed;
        $postFd = $postFdModel->getAll($request->readed,$request->freeText,$request->fbID);
        return response()->json($postFd);
    }

    /**
     * Xem thông tin tinfeedback từ client
     */
    function postFeedkbackInfo() {
